...
void assertFacts()
{
	...
	while (*p != '\0')
	{
		/* Create a query of the form "fact(<attribute>, <value>)" */
...
//		doublechar1(valbuf, '$');
	...
	fflush(debugFile);
		...
		if (strlen(p) > 0) rc = lsAssertaStr(CurEng, factbuf);
...
#ifdef TEST
	if (sSlash) printf(".exe Directory = %s\n", cgidir);
	else printf("No directory for .exe\n");
	printf(".exe Filename = %s\n", exename);
	printf(".xpl Filename = %s\n", xplname);
#endif
